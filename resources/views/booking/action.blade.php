...
<div id="booking-action">
  ...
  <post-ajax :post-to="'{{ urlTenant("bookings/confirm/{$booking->book_id}") }}'" reload-on-complete="1">{{ Form::button(trans('form.confirm'), ['class' => 'btn btn-primary btn-sm', 'style'=>'background: #00ceff; border: #00ceff']) }}</post-ajax>
  <post-ajax :post-to="'{{ urlTenant("bookings/cancel/{$booking->book_id}") }}'" reload-on-complete="1">{{ Form::button(trans('form.cancel_booking'), ['class' => 'btn btn-primary btn-sm', 'style'=>'background: #636b6f; border: #636b6f']) }}</post-ajax>
  @endif
  </div>
  <br/>
