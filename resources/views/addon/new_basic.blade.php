...
<div id="addon-new">
  <form-ajax action = "{{ urlTenant('addons/new') }}" method="POST" :go-to-next="gotonext" @startwait="startWait" @endwait="endWait" :reload-on-complete="reloadoncomplete">
    ...
    <div class="row">
      {{ Form::bsDate('add_date', trans('addon.add_date'), $resource_type->rty_id == 4 && !empty($booking) ? Carbon\Carbon::parse($booking->book_from)->format('d-m-Y 8:30') : today('d-m-Y 8:30'), ['class' => 'datetimepicker form-control']) }}
